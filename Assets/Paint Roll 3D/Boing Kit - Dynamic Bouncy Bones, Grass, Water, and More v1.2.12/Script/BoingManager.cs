...
namespace BoingKit
{
  public static class BoingManager
  {
    ...
    private static bool s_renderEventsSubscribed = false;
    private static void ValidateManager()
    {
      ...
      if (!s_renderEventsSubscribed)
      {
        Camera.onPreCull += BoingManagerPreRender;
        Camera.onPostRender += BoingManagerPostRender;

        #if UNITY_2019_1_OR_NEWER
        RenderPipelineManager.beginFrameRendering += BoingManagerPreRender;
        RenderPipelineManager.endFrameRendering += BoingManagerPostRender;
        #endif
    ...
    // stupid workaround for duplciate pre-render calls from Unity editor
    private static HashSet<Camera> s_preRenderHandledCameras = new HashSet<Camera>();
    private static HashSet<Camera> s_postRenderHandledCameras = new HashSet<Camera>();
    private enum NextRenderCallback { PreRender, PostRender }
    private static NextRenderCallback s_nextRenderCallback = NextRenderCallback.PreRender;
    private static int s_lastReactorFieldDispatchedFrame = -1;
    private static int s_lastPreRenderUpdateFrame = -1;
    ...
    internal static void LateUpdate()
    {
      ...
      // pull results here for the first time within a frame
      // so the skinning matrices can be built properly between LateUpdate and PreRender
      // additional results pulls are performed in PreRender if called more than once per frame
      PullBehaviorResults();
      PullReactorResults();
      ...
      s_preRenderHandledCameras.Clear();
      s_postRenderHandledCameras.Clear();
    }
    ...
    #if UNITY_2019_1_OR_NEWER
    internal static void BoingManagerPreRender(ScriptableRenderContext context, Camera[] camera)
    {
      BoingManagerPreRender(null);
    }

    internal static void BoingManagerPostRender(ScriptableRenderContext context, Camera[] camera)
    {
      BoingManagerPostRender(null);
    }
    #endif
